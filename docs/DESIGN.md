...
### 2026-09-07 存储契约修订（优先于下文旧链路描述）

世界快照使用 APS KV `scope=tool`、key `truman:run:world`，按用户与 Executa 隔离。
插件反向 RPC 不允许 `scope=app`；该范围只属于 App Host API。
新增第 9 个只读 action `get_snapshot`，直接读取已持久化快照，返回 `{value: snapshot|null}`。
前端 `refresh()` 经 `world/get_snapshot` 读取同一快照，不再调用默认 App 范围的 `anna.storage.get`。
读取不调用模型、不推进世界、不创建影子状态；存储错误原样冒泡。相同用户使用同一 Executa 的多个 App 将共享这个世界。
旧 App 范围数据保持原状，不自动迁移或回退；工具范围无快照时显示开镇入口。

模型输出校验同步收紧：决策必须符合 events 字段及动作字段类型，保留已知的裸数组兼容；
叙事必须同时包含非空字符串 story/cliffhanger。未闭合 think 块中的 JSON 不能用作答案。
不合规输出只允许一次模型纠正重试，耗尽后明确失败，不用空动作或空悬念兜底。
...
当前实现的提交边界（2026-09-21）：每个 tick 在临时 WorldState 副本上完成推进、注入、决策及日终叙事，随后一次保存 APS 快照；保存返回成功后才更新运行中的世界。失败不推进本地时钟、不丢失注入或历史，不提前提交语言切换；异常继续冒泡。多 tick 按单 tick 提交，后续失败不撤销此前成功的 tick。临时副本不持久化为第二份存档。若远程保存实际成功但响应丢失，结果仍不确定；当前没有分布式 exactly-once 或跨进程幂等重试保证。
...
**导演监视器**(v0.4.1 起实装;无构建步骤、ES module 约定不变)。一屏即全镇:舞台是画面,故事是画外音,导演只留一行输入。

```
┌ 顶栏: 楚门镇 ●REC 第 N 天 · HH:MM · tXXX ▸ ▸▸ ┐   ← 时间码 = 世界时钟本体
│ ┌───────────────舞台───────────────┐ ┌─今日故事─┐ │
│ │ 建筑剪影 + 发光居民点(悬浮提示)   │ │ 衬线散文  │ │   ← 故事是主角(§3.6)
│ │ 注入瞬间:扫光 + 下三分之一字幕条  │ │ +cliff-  │ │
│ │ 空舞台:STANDBY / 开镇 / 地平线    │ │ hanger   │ │
│ └───────────────────────────────────┘ ├─字幕─────┤ │
│                                        │ 事件流,  │ │   ← 戏点高亮(§3.6)
│                                        │ 新条目   │ │     SUB_MARK 符号
│                                        │ 滑入     │ │
└ 导演栏: [下一场戏,由你说了算……] [🎬 注入] ┘   ← 唯一的文字输入
第一幕: 全屏章节卡(壹/贰/叁 + 静场开场)             ← init 后弹一次
居民档案: 点居民点 → 模态(目标/关系/近事 + 熟悉度条)
```
...
- **动效层(第一波 d4b49f1 / 第二波 dd717b0 / 第三波 15100df)**:天色时相(舞台背景随 world_time 在 night/dawn/noon/dusk 四档渐变间 12s 流动);居民点呼吸 glow(按 agent_id 哈希播种负延迟,talk 快脉冲);章节卡 stagger 入场;时间码 tc-roll 翻动(值变化才重绘);建筑灯火(夜间小窗琥珀闪烁,位置/周期按 loc.id 哈希);舞台天气(world_change 含雨/雪关键词时真下雨/雪粒子,扫最近 4 条注入);故事纸展开 + cliffhanger 打字机;字幕按 tick 分组(最新 3 组平铺,更早折叠);看戏模式(⛶ 收起面板舞台全宽);居民 hover 实时状态浮卡(姓名/活动/位置/最近事件,数据随 refresh 自动更新);`prefers-reduced-motion` 全局降级。**原则:运动是叙事装置不是装饰**。
